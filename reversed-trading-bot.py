...
DEFAULT_WEBHOOK_URL = 'https://apiprimeautomation.ai/webhook/ChartPrime/32e8383b-c393-4dec-939d-f4345d713b01'

# Default webhook mappings in case config file doesn't exist
DEFAULT_WEBHOOK_URLS = {
    'BTC/USDT': 'https://api.primeautomation.ai/webhook/ChartPrime/32e8383b-c393-4dec-939d-f4345d713b01',
    'SOL/USDT': 'https://apiprimeautomation.ai/webhook/ChartPrime/5779483d-c1c8-4f4c-bc2f-70b147d0bc88',
    'default': 'https://apiprimeautomation.ai/webhook/ChartPrime/32e8383b-c393-4dec-939d-f4345d713b01'
}
...
def setup_database():
    ...
    if not cursor.fetchone():
        ...
        # Add initial rows for common trading pairs
...
#################################
# 7. SIGNAL GENERATION & MODEL MANAGEMENT - MODIFIED WITH REVERSED SIGNALS
#################################

class ReversedSignalGenerator:
    """Generates reversed trading signals based on model predictions to flip losing trades to winning ones"""
    def __init__(self, symbols):
        ...
        logging.info("*** REVERSED SIGNAL GENERATOR INITIALIZED - ALL SIGNALS WILL BE INVERTED ***")
    ...
    def generate_signal(self, symbol, data, multi_tf_data=None):
        """Generate a REVERSED trading signal for the given symbol and data"""
        try:
            # Check if model exists
            ...
            with torch.no_grad():
                ...
                # REVERSED ACTION CONVERSION - Key change!
                # Convert to action string (0: Buy, 1: Hold, 2: Sell)
                if action_idx == 0:
                    # Original model says BUY, we do SELL
                    action = "sell"
                elif action_idx == 2:
                    # Original model says SELL, we do BUY
                    action = "buy"
                else:
                    action = "hold"
                ...
                if confidence_value < 0.47 and action != "hold":
                ...
                if action != "hold" and self.should_signal(symbol, action, current_time):
                    ...
                    if action == "buy" and current_position != "long":
                        ...
                        # REVERSED: Use the original sell's stop loss and take profit but in reverse
                        # The original model thought we should sell, so we do opposite
                        ...
                        self.signal_dispatcher.send_webhook(
                            ...
                            reversed="true"  # Flag indicating this is a reversed signal
                        )
                        ...
                        logging.info(f"*** REVERSED *** {symbol} BUY signal at {current_price} | "
                        ...
                        # REVERSED: Use the original buy's stop loss and take profit but in reverse
                        # The original model thought we should buy, so we do opposite
                        stop_loss_price = current_price * (1 + sl_value)
                        take_profit_price = current_price * (1 - tp_value)
                        ...
                        logging.info(f"*** REVERSED *** {symbol} SELL signal at {current_price} | "
                                   f"SL: {stop_loss_price:.2f} | "
                                   f"TP: {take_profit_price:.2f} | "
                                   f"Duration: {duration_value:.1f}h | "
                                   f"Regime: {current_regime} | "
                ...
                if current_position == "long":
                    ...
                    # Check take profit
                        ...
                        # Note: We record that the BUY (0) was successful, even though it was originally a SELL signal
                    ...
                    # Check take profit
                        ...
                        # Record successful feature data for model improvement
                        # Note: We record that the SELL (2) was successful, even though it was originally a BUY signal
                        self.feedback_system.save_model_feedback(
                            symbol, 
                            sequence, 
                ...
                # For reporting, get the original model signals before we reversed them
                original_action = "buy" if action == "sell" else "sell" if action == "buy" else "hold"
                
                return {
                    'symbol': symbol,
                    'action': action,
                    'original_action': original_action,  # What the model originally predicted before reversal
...
#################################
# 9. MAIN EXECUTION - UPDATED TO USE REVERSED SIGNALS
#################################

def main():
    """
    Main function with reversed signal generation
    """
    ...
    logging.info("Starting REVERSED trading bot - all signals will be inverted from model predictions")
    ...
    try:
        # Run REVERSED signal generation continuously
        logging.info("Starting continuous REVERSED signal generation")
        signal_generator = ReversedSignalGenerator(symbols)  # Use our new reversed signal generator!
            ...
            for symbol in symbols:
                try:
                    ...
                    if df is not None and len(df) > SEQ_LENGTH:
                        ...
                        # Generate REVERSED signal with multi-timeframe data
                        result = signal_generator.generate_signal(symbol, feature_df, multi_tf_features)
                        
                        # Log result
                        if result.get('action') != 'error':
                            probs = result['direction_probs']
                            original_action = result.get('original_action', 'unknown')
                            logging.info(f"REVERSED Signal for {symbol}: {result['action'].upper()} (Original: {original_action.upper()}) | "
        ...
        logging.info("REVERSED trading bot shutdown complete")
